...
public class HibernatePatientDAO implements PatientDAO {
    ...
    public List<Patient> searchPatient(String name) throws ParseException {
        ...
        org.apache.lucene.search.Query query = parser.parse("names.givenName:" + name + "~");
        FullTextQuery textQuery = session.createFullTextQuery(query, Person.class);
        return textQuery.list();
    }

    public List<Patient> searchPatientByIdentifier(String identifier) throws ParseException {
        FullTextSession session = Search.getFullTextSession(this.sessionFactory.getCurrentSession());
        QueryParser parser = new QueryParser("givenName", new StandardAnalyzer());
        org.apache.lucene.search.Query query = parser.parse("names.givenName:" + identifier + "~");
        FullTextQuery textQuery = session.createFullTextQuery(query,Person.class);
        return textQuery.list();
    }
